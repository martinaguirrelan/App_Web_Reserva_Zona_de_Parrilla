...
def _get_viewable_url(stored_url: str) -> str:
    """Reescribe la URL almacenada para apuntar al bucket correcto.
    El bucket es público, por lo que la URL directa funciona sin signed URLs."""
    if not (settings.use_supabase_storage and settings.supabase_url):
        return stored_url

    bucket = settings.supabase_storage_bucket

    # Si la URL ya apunta al bucket correcto, devolverla tal cual
    if f"/object/public/{bucket}/" in stored_url:
        return stored_url

    # Si la URL apunta a un bucket distinto (ej: "vouchers" → "comprobantes"),
    # reescribir el segmento del bucket para corregirlo
    import re
    corrected = re.sub(
        r"/object/public/[^/]+/",
